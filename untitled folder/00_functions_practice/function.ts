...
function destroyLongestArrays(arr: number[][]): number[][] {
  let longest = 0;

  for (let i = 0; i < arr.length; i++) {
    let current_element = arr[i];
    let length = current_element.length;
    if (longest < length) {
      longest = length;
    }
  }

  let result = arr.filter((array) => {
...
// console.log(
//   destroyLongestArrays([[1, 2, 3, 6], [2, 3, 4], [2, 4, 4, 6], [8], [3, 9]])
// );
...
function togglePower(arr: some[], id: number): some[] {
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].id == id) {
...
function reverseSentence(s:string){
